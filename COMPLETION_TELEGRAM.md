...
> This is the Telegram adaptation of the original Discord spec (`COMPLETION.md`). The **domain**
> (grading/scoring/settlement), the **provider** layer (API-Football), the **request budget**,
> and the **database** are interface-agnostic and carry over essentially unchanged. Only the
> **bot interaction layer** and the **config secrets/IDs** are rewritten for Telegram. Where this
> document differs from `COMPLETION.md`, **this document wins** for the Telegram build.
...
```
/ralph-loop "Build TigrinhoDaCopa (Telegram) per COMPLETION_TELEGRAM.md, one milestone increment
...
scoring, or grading rules MUST update `/ajuda` text **and** this `COMPLETION_TELEGRAM.md` in the same
change.
