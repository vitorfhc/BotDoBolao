...
def _provider(
    usage: ApiUsageRepository, handler: Handler, *, cap: int = 100
) -> ApiFootballProvider:
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler), base_url="http://test")
    budget = RequestBudget(usage, cap=cap, reset_tz=UTC, clock=lambda: NOW)
    return ApiFootballProvider(
        league_id=1, season=2026, budget=budget, client=client, clock=lambda: NOW
    )
...
async def test_get_live_results_filters_to_configured_league(usage: ApiUsageRepository) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        items = [
            {
                "fixture": {
    ...
    provider = _provider(usage, handler)
    results = await provider.get_live_results()
    assert [r.fixture_id for r in results] == [1]
